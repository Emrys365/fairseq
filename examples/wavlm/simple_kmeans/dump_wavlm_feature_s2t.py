...
from fairseq.data.audio.audio_utils import read_from_stored_zip
...
class WavLMFeatureReaderS2T(WavLMFeatureReader):
    def read_audio(self, path, ref_len=None):
        path, *extra = path.split(":")
        assert len(extra) == 2
        assert path.endswith(".zip")

        data = read_from_stored_zip(path, int(extra[0]), int(extra[1]))
        f = io.BytesIO(data)
...
def main(
    ...
    reader = WavLMFeatureReaderS2T(ckpt_path, layer, max_chunk)
