...
def load_audio(manifest_path, uid_path, sid_path, max_keep, min_keep):
    ...
    return root, names, uids, sids, inds, tot, sizes
...
class WavLMDataset(FairseqDataset):
    def __init__(
        self,
        manifest_path: str,
        ...
        audio_root, audio_names, uids, sids, inds, tot, sizes = load_audio(
            manifest_path,
            manifest_uid_path,
            manifest_sid_path,
    ...
    def __getitem__(self, index):
        ...
        L = min(np.random.randint(1, len(wav) // 2 + 1), len(noise))
