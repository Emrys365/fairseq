...
from fairseq.data import Dictionary, WavLMDataset, HubertDataset, TsHubert2MixDataset
...
class WavLMPretrainingConfig(FairseqDataclass):
    ...
    # TSE finetuning related
    fine_tuning_mix: bool = field(
        default=False, metadata={
            "help": "set to true if fine-tuning WavLM on mixed speech"
        }
    )
...
class WavLMPretrainingTask(FairseqTask):
    ...
    def __init__(
        ...
        is_fine_tuning = cfg.fine_tuning or cfg.fine_tuning_mix
    ...
    def load_dictionaries(self):
        ...
        is_fine_tuning = self.cfg.fine_tuning or self.cfg.fine_tuning_mix
        return dictionaries[0] if is_fine_tuning else dictionaries
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        is_fine_tuning = self.cfg.fine_tuning or self.cfg.fine_tuning_mix
        dicts = [self.target_dictionary] if is_fine_tuning else self.dictionaries
        ...
        elif self.cfg.fine_tuning_mix:
            manifest_enroll = f"{self.cfg.data}/{split}.emb"
            manifest_utt2enroll = f"{self.cfg.data}/{split}.utt2emb.json"
