...
from fairseq.models.wavlm.encoder import ConvFeatureExtractionModel, TransformerEncoder
from fairseq.tasks.wavlm_pretraining import WavLMPretrainingConfig, WavLMPretrainingTask
...
class WavLM(BaseFairseqModel):
    def __init__(
        self,
        cfg: WavLMConfig,
        task_cfg: Optional[WavLMPretrainingConfig] = None,
    ...
    @classmethod
    def build_model(cls, cfg: WavLMConfig, task: WavLMPretrainingTask):
        """Build a new model instance."""

        if task is not None:
            model = WavLM(cfg, task.cfg, task.dictionaries)
        else:
            model = WavLM(cfg)
        return model
