...
@dataclass
class WavLMCtcConfig(WavLMAsrConfig):
    pass


@register_model("wavlm_ctc", dataclass=WavLMCtcConfig)
class WavLMCtc(BaseFairseqModel):
    def __init__(self, cfg: WavLMCtcConfig, w2v_encoder: BaseFairseqModel):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: WavLMCtcConfig, task: FairseqTask):
...
@dataclass
class WavLMSeq2SeqConfig(WavLMAsrConfig):
...
@register_model("wavlm_seq2seq", dataclass=WavLMSeq2SeqConfig)
class WavLMSeq2SeqModel(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder):
        super().__init__(encoder, decoder)

    @classmethod
    def build_model(cls, cfg: WavLMSeq2SeqConfig, task: FairseqTask):
        """Build a new model instance."""
        ...
        model = WavLMSeq2SeqModel(encoder, decoder)
    ...
    @classmethod
    def build_encoder(cls, cfg: WavLMAsrConfig, task):
        return WavLMEncoder(cfg, task)

    @classmethod
    def build_decoder(cls, cfg: WavLMSeq2SeqConfig, tgt_dict, embed_tokens):
...
class WavLMEncoder(FairseqEncoder):
    def __init__(self, cfg: WavLMAsrConfig, task):
