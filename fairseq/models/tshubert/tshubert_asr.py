...
TSE_MODE_CHOICES = ChoiceEnum(["none", "add", "cat", "film", "cln"])


@dataclass
class TsHubertAsrConfig(FairseqDataclass):
...
@dataclass
class TsHubertCtcConfig(TsHubertAsrConfig):
    pass


@register_model("tshubert_ctc", dataclass=TsHubertCtcConfig)
class TsHubertCtc(BaseFairseqModel):
    def __init__(self, cfg: TsHubertCtcConfig, w2v_encoder: BaseFairseqModel):
        super().__init__()
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: TsHubertCtcConfig, task: FairseqTask):
        """Build a new model instance."""
        w2v_encoder = TsHubertEncoder(cfg, task)
...
@dataclass
class TsHubertSeq2SeqConfig(TsHubertAsrConfig):
    # Encoder
...
@register_model("tshubert_seq2seq", dataclass=TsHubertSeq2SeqConfig)
class TsHubertSeq2SeqModel(FairseqEncoderDecoderModel):
    def __init__(self, ssl_encoder, encoder, decoder):
        super().__init__(encoder, decoder)
        self.ssl_encoder = ssl_encoder

    @classmethod
    def build_model(cls, cfg: TsHubertSeq2SeqConfig, task: FairseqTask):
        """Build a new model instance."""
        ...
        model = TsHubertSeq2SeqModel(ssl_encoder, encoder, decoder)
    ...
    @classmethod
    def build_ssl_encoder(cls, cfg: TsHubertAsrConfig, task):
        return TsHubertEncoder(cfg, task)

    @classmethod
    def build_encoder(cls, cfg: TsHubertSeq2SeqConfig, input_dim: int):
        return MixTransformerEncoder(cfg, input_dim)

    @classmethod
    def build_decoder(cls, cfg: TsHubertSeq2SeqConfig, tgt_dict, embed_tokens):
...
class TsHubertEncoder(FairseqEncoder):
    def __init__(self, cfg: TsHubertAsrConfig, task):
        ...
        elif self.spk_adapter_type != "none":
    ...
    def forward(self, source, enrollment, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            feat, feat_aux, feat_pen, _, padding_mask = self.w2v_model.forward_conv(
                source, enrollment, padding_mask=padding_mask
            )
        if self.spk_adapter_type != "none":
            spk_emb = kwargs["enrollment_emb"]
        if self.spk_adapter_type == "add":
        ...
        with torch.no_grad() if not ft and self.spk_adapter_type == "none" else contextlib.ExitStack():
...
class MixTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, cfg: TsHubertSeq2SeqConfig, input_dim: int):
        super().__init__(None)
